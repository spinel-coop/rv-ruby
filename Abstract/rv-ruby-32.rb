...
class RvRuby32 < Formula
  ...
  def install
    ...
    if ENV.key?("HOMEBREW_BASERUBY")
      ...
      odie "HOMEBREW_BASERUBY must contain the path to a ruby #{version} executable" unless File.exist?(baseruby)

      baseruby_version = baseruby && `#{baseruby} -v`
      if /#{Regexp.escape(version)}/.match?(baseruby_version)
    ...
    args += %w[--enable-libedit] if OS.mac?
